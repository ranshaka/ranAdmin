<!-- 
    源码地址 https://gitee.com/WangWenCong/MyVueComponents/blob/main/myColorSelect/MyColorSelect.vue#
     抄了一下 王文聪 作业，在源码地址上进行了二次开发
    如有侵权，请连续删除;
 -->
<template>
  <div class="colorPicker">

    <a-dropdown :trigger="['click']" v-model:visible="viewFlag" >
      <div class="colorBox">
        <!-- @blur="viewFlag=false"  @mousedown.stop.prevent -->
        <div type="button" class="colorView" :style="'background:'+modelValue"></div>
      </div>
      <template #overlay  >
        <div class="color-select"  @mousedown.stop.prevent  @visibleChange="()=>{visible= 'visible'}">
          ...
          <ul class="preset">
            <li v-for="item in preset" :key="item" :style="`background-color: ${item}`" @click="presetChange(item)">
            </li>
          </ul>
          <div class="color-Box">
            <div class="color-Bottom" style="background:#f60;color:#FFF;" @click="viewFlag=false">
              关闭
            </div>
            <div class="color-Bottom" style="background:aqua;" @click="Reset">
              默认
            </div>
            <div class="color-Bottom" @click="checkedColor">
              确定
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    name: 'MyColorSelect',
    model: {
      prop: 'color', // 自定义prop属性
      event: 'colorChange' // 自定义它的触发方法名
    },
    props: {
      modelValue: {
        type: Object || String,
    ...
    setup(props, context) {
      ...
      const checkedColor = () => {
        context.emit('update:modelValue', rgba2hex(red.value, green.value, blue.value, alpha.value))
        viewFlag.value = false
      }
      const Reset = () => {
        context.emit('update:modelValue', props.result || rgba2hex(red.value, green.value, blue.value, alpha.value))
        viewFlag.value = false
      }
      const refs = {
      ...
      function handleChangeSV(e) {
        ...
        let {
          ...
          g,
          b
        } = hsv2rgb(hue.value, saturation.value, value.value);
        red.value = r;
        green.value = g;
        blue.value = b;
        // 移动背景板圆圈
